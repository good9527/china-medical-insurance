...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/测试用例-6%2C804%20Passed-blue?style=for-the-badge&logo=jest" alt="Tests">
...
**全国医疗保险待遇测算平台**是一个面向全国参保人的民生级开源工具与政策数据库。本项目完整收录并结构化建模了中国内地 **31 个省、自治区、直辖市及新疆生产建设兵团下辖的全部 344 个医保统筹区**现行有效的基本医疗保险待遇政策。

平台不仅支持在手机端、网页端即时进行精确到分、严丝合缝的医保报销金额估算，更建立了一整套**官方红头文件溯源体系**与**多维智能巡检引擎**，杜绝政策滞后、倒挂与非标误填。
...
2. **多层次报销测算引擎 (`src/engine/calculator.ts`)**：
...
│   │   └── ...                 # 涵盖全国各省自治区
│   ├── engine/                 # 核心计算与质检引擎
│   │   ├── calculator.ts       # 核心报销计算器（门诊、住院、大病、异地）
│   │   ├── inspector/          # 智能体检引擎
...
| 指令 | 说明 |
| :--- | :--- |
| `npm run check:all` | **【完整门禁】** 顺序执行合规审计 + 智能体检 + 官网监控 + 6804 项场景测试断言 + H5 构建 |
| `npm run audit:smart` | **【全量体检】** 对全国 344 个统筹区执行全量多维智能规则扫描 |
| `npm run audit:patrol` | **【随机巡查】** 随机抽取 35 个统筹区进行深度测算与倒挂抽检 |
| `npm run test:calc` | **【场景测试】** 运行 6,804 项全域门诊、住院自动化测算断言 |
